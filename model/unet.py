...
class UNet(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.time_linear = nn.Linear(in_ch, in_ch)

    def forward(self, x, time_emb=None):
        #获取模型当前设备
        device = x.device  # 获取x的设备，假设x在正确的设备上
        # 确保 time_emb 也在同一设备上
        time_emb = time_emb.to(device) if time_emb is not None else None

        leng = x.shape[-1]
        time_embed = self.time_linear(time_emb)
        time_embed = time_embed.unsqueeze(2).unsqueeze(3).repeat(1, 1, leng, leng)
        # x = x + time_embed
        x = x.to(device) + time_embed.to(device)
